    ...
    <div class="card card-custom">
        <div class="flex-wrap pb-0 border-0 card-header">
            <div class="card-title">
                <h3 class="card-label">عدد موظفين العقود: {{ App\Models\contract1::count() }} </h3>
            ...
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ url('/contract/create') }}" class="btn btn-primary font-weight-bolder">
            ...
            <table id="example" class="table dt-responsive" style="width:100%; padding:0%">
                <thead class="thead_dark">
                    <tr>
                        ...
                        <th width="5%">{{ 'البريد الالكتروني ' }}</th>
                        <th width="4%">{{ 'الجنس ' }}</th>
                        ...
                        <th width="6%">{{ 'الاجراءات ' }} </th>
                    </tr>
                </thead>

                <tbody>
                    @foreach($contract1 as $contract1s)
                    <tr>
                        {{-- <td>{{ $contract1s->id }}</td> --}}
                        <td>{{ $contract1s->contract_num }}</td>
                        <td>{{ $contract1s->id_contract }}</td>
                        <td>{{ $contract1s->contract_name }}</td>
                        <td>{{ $contract1s->contract_email }}</td>
                        <td>{{ $contract1s->contract_gender }}</td>
                        <td>{{ $contract1s->contract_status }}</td>
                        <td>{{ $contract1s->contract_address }}</td>
                        <td>{{ $contract1s->contract_mobile }}</td>
                        <td>{{date('Y-m-d', strtotime($contract1s->contract_birthdate )) }}</td>
                        <td>{{ $contract1s->contract_degree }}</td>
                        ...
                        <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                            ...
                            <span style="overflow: visible; position: relative; width: 110px;">
                                ...
                                <a href="{{ url('/contract/' . $contract1s->id) }}" title="View court"><button
                                ...
                                <form method="POST" action="{{ url('/contract' . '/' . $contract1s->id) }}"
                                ...
                                <a href="{{ url('/contract/' . $contract1s->id . '/edit') }}"
                                ...
                                {{-- <div class="dropdown dropdown-inline"> <a href="javascript:;"
                                    ...
                                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                        <ul class="nav nav-hoverable flex-column">
                                            <li class="nav-item"><a class="nav-link" href="{{ url('/contract/' . $contract1s->id) }}"><i
                                                        class="nav-icon la la-edit"></i><span class="nav-text"> بيانات العقد</span></a></li>
