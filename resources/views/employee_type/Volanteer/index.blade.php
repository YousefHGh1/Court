    ...
    <div class="card card-custom">
        <div class="flex-wrap pb-0 border-0 card-header">
            <div class="card-title">
                <h3 class="card-label">عدد المتدربين/المتطوعين: {{ App\Models\volanteer::count() }} </h3>
            ...
            <div class="card-toolbar">
                <!--begin::Button-->
                <a href="{{ url('/volanteer/create') }}" class="btn btn-primary font-weight-bolder">
        ...
        <div class="card-body">
            ...
            <table id="example" class="table dt-responsive" style="width:100%; padding:0%">
                <thead class="thead_dark">
                    <tr>
                        <th width="4%">{{ 'رقم المتدرب' }}</th>
                        <th width="5%">{{ 'رقم الهوية ' }}</th>
                        <th width="10%">{{ 'الاسم ' }}</th>
                        <th width="10%">{{ 'البريد الالكتروني ' }}</th>
                        <th width="4%">{{ 'الجنس ' }}</th>
                        <th width="5%">{{ 'الحالة الاجتماعية ' }}</th>
                        <th width="10%">{{ 'العنوان ' }}</th>
                        <th width="5%">{{ 'رقم الموبايل ' }}</th>
                        <th width="6%">{{ 'تاريخ الميلاد ' }}</th>
                        <th width="5%">{{ 'المؤهل العلمي ' }}</th>
                        <th width="6%">{{ 'التخصص ' }}</th>
                        <th width="5%">{{ 'سنة التخرج ' }}</th>
                ...
                <tbody>
                    ...
                    <tr>
                        <td>{{ $volanteers->id }}</td>
                        <td>{{ $volanteers->id_volanteer }}</td>
                        <td>{{ $volanteers->volanteer_name }}</td>
                        <td>{{ $volanteers->volanteer_email }}</td>
                        <td>{{ $volanteers->volanteer_gender }}</td>
                        <td>{{ $volanteers->volanteer_status }}</td>
                        <td>{{ $volanteers->volanteer_address }}</td>
                        <td>{{ $volanteers->volanteer_mobile }}</td>
                        <td>{{date('Y-m-d', strtotime($volanteers->volanteer_birthdate )) }}</td>
                        <td>{{ $volanteers->volanteer_degree }}</td>
                        <td>{{ $volanteers->volanteer_major }}</td>
                        <td>{{ $volanteers->volanteer_graduation }}</td>
                        ...
                        <td data-field="Actions" data-autohide-disabled="false" aria-label="null"
                            ...
                            <span style="overflow: visible; position: relative; width: 110px;">
                                ...
                                <a href="{{ url('/volanteer/' . $volanteers->id) }}" title="View court"><button
                                ...
                                <form method="POST" action="{{ url('/volanteer' . '/' . $volanteers->id) }}"
                                ...
                                <a href="{{ url('/volanteer/' . $volanteers->id . '/edit') }}"
                                ...
                                {{-- <div class="dropdown dropdown-inline"> <a href="javascript:;"
                                    ...
                                    <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                        <ul class="nav nav-hoverable flex-column">
                                            <li class="nav-item"><a class="nav-link" href="{{ url('/volanteer/' . $volanteers->id) }}"><i
                                                        class="nav-icon la la-edit"></i><span class="nav-text"> بيانات المتدرب/المتطوع</span></a></li>
